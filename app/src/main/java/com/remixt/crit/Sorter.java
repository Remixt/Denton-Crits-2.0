...
class Sorter {
    ...
    private int groupOnePages = 0;
    private int groupTwoCount = 0;
    private int groupTwoPages = 0;
    private int alterGroupNum = 1;

    ArrayList<Person> sort(ArrayList<Person> people) {
        ...
        while (!pmap.isEmpty()) {
            ...
            if (groupOnePages < groupTwoPages) { // try to keep the groups within 2 pages if possible
                if (groupOneCount > groupTwoCount + 1 && groupOnePages > 0) { // don't separate group numbers by more than 2 people at any given time
                    person.setGroup(2);
                    addToGroupCount(2);
    ...
    private void addGroupPages(int groupNum, int numPages) {
        switch (groupNum) {
            case 1:
                groupOnePages += numPages;
                break;
            default:
                groupTwoPages += numPages;
        }
    }
}